 ...
 * 出力画像の最大ピクセル数。
 *
 * 理由: iOS Safari は canvas の総面積に上限があり（概ね 16.7MP 相当）、
 * それを超えると canvas が空 (透明) になる。補正後の画像がこれを超える
 * 場合のみ、四隅から算出した理想サイズを等比で縮める。
 * 通常のスマートフォン写真では、補正後は元画像より小さくなるため発動しない。
 */
export const MAX_OUTPUT_PIXELS = 16_700_000;

export interface LoadedImage {
  /** EXIF Orientation 適用済み・原寸の画像。以降の処理はすべてこれを基準にする。 */
  canvas: HTMLCanvasElement;
  width: number;
  height: number;
  /** 元ファイルの画素数に対して縮小した場合の倍率 (1 = 無縮小)。 */
  scale: number;
  fileName: string;
  /** 元ファイルの MIME。保存形式の初期値決定に使う。 */
  mimeType: string;
}
 ...
 *
 * iPhone から選んだ画像は MIME が空文字になることがあるため、
 * File.type だけに頼らずマジックナンバーも見る。
 * HEIC のデコードは Safari 以外のブラウザでは非対応で、対応するには
 * 数 MB の wasm デコーダが必要になるため、初版では明示的に非対応とする。
 */
...
export async function loadImageFile(file: File): Promise<LoadedImage> {
  if (await isHeic(file)) {
    // Safari は HEIC をデコードできるが、他ブラウザとの体験差が大きいため
    // まずデコードを試し、失敗したら HEIC 固有のメッセージを出す。
    try {
      return await decodeToCanvas(file, await decode(file));
    } catch {
  ...
  return decodeToCanvas(file, decoded);
}
...
function decodeToCanvas(
  ...
  const scale = pixels > MAX_INPUT_PIXELS ? Math.sqrt(MAX_INPUT_PIXELS / pixels) : 1;
  ...
  // ImageBitmap は GPU/CPU メモリを持つので、canvas へ写したら即解放する。
